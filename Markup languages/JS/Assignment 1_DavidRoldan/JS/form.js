...
function validateSurname() {
    ...
    if (surname.value.length < 2 || surname.value.length > 15) {
        return listErrors.push("The surname must be between 2 to 15 characters")
    }
}
...
function validateAge() {
    var dateBirth = new Date(document.getElementById("dateofbirth").value);
    var todayDate = new Date();
    var age = todayDate.getFullYear() - dateBirth.getFullYear();
    if (age < 18) {
        listErrors.push("You need to be older than 18 years old")
    }
}
